...
---
### 10. 組合層 (`portfolio.py`)
- [ ] 實作雙帳本初始化：capital_A = 50%, capital_B = 50%
- [ ] 實作兩帳本獨立記帳、獨立持倉
- [ ] 實作季度再平衡邏輯：賣賺的、補虧的，回到 50/50
- [ ] 實作 DCA 現金流：每月入金按 50/50 分進兩個帳本 cash_pool
- [ ] 實作滾動相關係數監控（60 日窗口）
- [ ] 實作相關性警告：長期 > 0.6 時輸出警告
- [ ] 實作合併淨部位邏輯（同一標的兩帳本同時持有時）
- [ ] 實作組合日淨值序列計算
- [ ] 單元測試：驗證再平衡後資金比例正確
- [ ] 單元測試：驗證 DCA 分配正確
---
...
- [ ] 跑 Phase 1 基準線：0050 純定期定額 B&H
- [ ] 跑 Phase 2：策略 A 單獨（日線版 VOO）
- [ ] 跑 Phase 2：策略 A 單獨（日線版 0050）
- [ ] 跑 Phase 2：策略 A 單獨（週線版 VOO）
- [ ] 跑 Phase 3：策略 B 單獨（VOO）
- [ ] 跑 Phase 3：策略 B 單獨（0050）— 確認淨期望值為正才啟用
- [ ] 跑 Phase 4：A+B 組合 50/50 + 季度再平衡（VOO）
- [ ] 跑 Phase 4：A+B 組合 50/50 + 季度再平衡（0050）
- [ ] 跑 Phase 5：敏感度測試 — 各參數 ±20% 績效是否穩健
- [ ] 跑 Phase 6 變體：槓桿版
- [ ] 跑 Phase 6 變體：債券輪動版
- [ ] 跑 Phase 6 變體：ADX 濾網版
---
### 13. 驗收標準確認
- [ ] 組合 MDD 明顯低於 B&H 基準
- [ ] 組合 Sharpe 高於 B&H 基準
- [ ] 組合 CAGR 不低於 B&H 的 85%
- [ ] Phase 5 敏感度測試通過（參數 ±20% 績效不崩）
- [ ] 兩帳本滾動相關係數 < 0.3（預期）
- [ ] 策略 B 跑 0050 淨期望值確認（若為負則停用 0050）