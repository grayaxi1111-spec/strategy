# 量化交易系統 Roadmap
> 依照 [AGENT.md](file:///Users/chengche/code/strategy/AGENT.md) 拆解，一個 Phase 完成所有開發與驗收。
> 做完一項就打 `[x]`。
---
## Phase 1: 完整系統開發與驗收
---
...
- [ ] 實作進場執行：t 日收盤判斷，標記 t+1 開盤執行（策略層只產生訊號，t+1 實際成交延遲屬於第 7 節回測引擎範疇，尚未實作）
...
- [ ] 實作 t 日收盤出訊號 → t+1 開盤成交的延遲邏輯
- [ ] 實作暖機期 200 個交易日不交易的限制
- [ ] 實作單策略回測迴圈：逐日跑策略 → 產生交易紀錄
- [ ] 實作交易紀錄 DataFrame（日期、方向、價格、部位、現金）
- [ ] 實作每日淨值計算（持倉市值 + 現金）
- [ ] 單元測試：驗證延遲一日成交的正確性
- [ ] 單元測試：驗證暖機期內無交易
---
### 8. 成本模型（整合進 `backtest.py` 或獨立模組）
- [ ] 實作 0050 買入成本：手續費 0.1425% × 折扣
- [ ] 實作 0050 賣出成本：手續費 0.1425% × 折扣 + 證交稅 0.1%
- [ ] 實作 VOO 買入成本：滑價 0.05%
- [ ] 實作 VOO 賣出成本：滑價 0.05%
- [ ] 成本參數從 `config.yaml` 讀取，可配置折扣比例
- [ ] 單元測試：驗證各標的來回成本計算正確
- [ ] ⚠️ 記錄警告：策略 B 跑 0050 來回成本約 0.3%，會吃掉 10~30% 獲利
---
### 9. 績效層 (`metrics.py`)
- [ ] 實作 CAGR（年化報酬率）計算
- [ ] 實作 MDD（最大回撤）計算
...
- [ ] 實作盈虧比計算（平均獲利 / 平均虧損）
- [ ] 實作年均交易次數計算
- [ ] 實作 Time in Market 比例計算
- [ ] 提供單一帳本的績效報表輸出
- [ ] 提供組合層的績效報表輸出
- [ ] 組合層額外：兩帳本滾動相關係數（60 日）
- [ ] 組合層額外：再平衡貢獻分析
- [ ] 單元測試：用已知報酬序列驗證各指標數值
---
### 10. 組合層 (`portfolio.py`)
